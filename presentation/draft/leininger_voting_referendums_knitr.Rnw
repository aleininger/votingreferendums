...
\section{Introduction}\label{sec:introduction}
    ...
    \Sexpr{refs$kt.ja[which(refs$anr == '552.1')]} voting in favor. } A more recent example is the anti-immigration initiative accepted by a slim majority of 50.3\% on 14 February 2014.
    
    I use the full cumulation of the VOX post-referendum surveys that cover \Sexpr{dim(projets)[1]} national referendums held in Switzerland between \Sexpr{min(projets$annee)} and \Sexpr{max(projets$annee)} (\Sexpr{((dim(projets)[1] / dim(refs)[1]) * 100) %>% round(.,1)} 
    ...
    \% of the \Sexpr{dim(refs)[1]} referendums held in that period.) to analyze the individual and macro-level determinants of voting `against the government.' In the period considered the government issued \Sexpr{table(refs$br.pos)[1]} recommendations to vote yes, \Sexpr{table(refs$br.pos)[2]} and one neutral.\footnote{The latter is not included in the analysis because there is no vote recommendation that voters could vote against.} The type of referendum practically determines the recommendation the government will issue. As Obligatory Referendum is triggered by government action, requiring confirmation of parliamentary decisions by the people, the government has advocated for a yes-vote in all \Sexpr{table(refs$br.pos,refs$rechtsform)[1, 1]} obligatory referendums and obviously also for all its \Sexpr{table(refs$br.pos,refs$rechtsform)[1, 4]} counter-proposals to an initiative. The government also advocated a yes-vote in all but one of  \Sexpr{table(refs$typ)['Facultative Referendum']} facultative referendums. Note that in a facultative referendum voters are asked whether they approve of the law passed by parliament. The government advocated for a no in all but two initiatives of \Sexpr{table(refs$typ)['Initiative']} referendums in total. Overall, in \Sexpr{table(refs$niederlage)['TRUE']} (\Sexpr{round((table(refs$niederlage)['TRUE'] / dim(refs)[1]) * 100, 1)}\%) of referendums in that period a mjaority of voters did not follow the government's vote recommendation.  
    ...
    \tiny\centering
      ...
      Lost & \Sexpr{tab_wonlost[2,1]} (\Sexpr{tab_wonlostprop[2,2]}\%) & \Sexpr{tab_wonlost[2,2]} (\Sexpr{tab_wonlostprop[2,2]}\%) & \Sexpr{tab_wonlost[2,3]} (\Sexpr{tab_wonlostprop[2,3]}\%) & \Sexpr{tab_wonlost[2,4]} (\Sexpr{tab_wonlostprop[2,4]}\%) \\
      \bottomrule
      \end{tabular}
...
\centering
<<echo=FALSE, fig.keep='high', cache=TRUE>>=
...
Observations ares clustered into referendums refered to as \textit{projets} and a referendum day -- on which voters may vote on multiple referendums -- referred to as \textit{scrutin}. \Sexpr{dim(projets)[1]} \textit{projets} clustered into \Sexpr{dim(scrutins)[1]} \textit{scrutins}. representative at the national level. 
...
From \Sexpr{min(scrutins$projets)} to \Sexpr{max(scrutins$projets)} projets per scrutin. On average \Sexpr{round(mean(scrutins$projets), 1)} referendums per scrutin, median is \Sexpr{median(scrutins$projets)}.

Of these \Sexpr{table(projets$typex)['Initiative']} are initiatives, \Sexpr{table(projets$typex)['Referendum']} are referendums, %facultative referendums?
\Sexpr{table(projets$typex)['Obligatorisches Referendum']} are obligatory referendums, while %remainder category
\ref{fig:typetopic}.



average sample size of \Sexpr{round(mean(projets$samplesize),0)} (SD: \Sexpr{round(sd(projets$samplesize),0)})

\begin{figure}
<<echo=FALSE, fig.keep='high'>>=
source('../../analysis/descriptives.R')
@
