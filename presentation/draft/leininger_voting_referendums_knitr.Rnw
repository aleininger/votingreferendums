...
\section{Introduction}\label{sec:introduction}
   ...
   For this purpose, I use the full cumulation of the VOX post-referendum surveys \citep{brunner_voxit:_????} that cover \Sexpr{dim(projets)[1]} national referendums held in Switzerland between \Sexpr{min(projets$annee)} and \Sexpr{max(projets$annee)} (\Sexpr{((dim(projets)[1] / dim(refs)[1]) * 100) %>% round(.,1)} \% of the \Sexpr{dim(refs)[1]} referendums held in that period).
